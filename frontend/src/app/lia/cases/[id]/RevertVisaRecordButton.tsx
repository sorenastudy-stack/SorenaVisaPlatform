...
// PR-LIA-8 — Two-step destructive un-issue of a visa record. Mirrors
// PR-LIA-7's RevertInzSubmissionButton: required reason (10–500 chars,
// encrypted on audit) + type-the-case-id confirmation. The visa file
// itself stays on disk (recoverable; cleanup job is a future concern,
// same pattern as PR-LIA-7).

export function RevertVisaRecordButton({
  caseId,
  outcome,
}: {
  caseId: string;
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await api.post(`/cases/${caseId}/visa/revert`, {
        reason: reason.trim(),
      });
      setOpen(false);
      reset();
      ...
      setError(e instanceof ApiError ? e.message : 'Failed to revert visa record.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-white shadow-xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <h2 className="text-lg font-bold text-red-800">Revert visa record</h2>
              </div>
            ...
            <p className="text-sm text-red-900 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4 leading-relaxed">
              This rolls the case back to <strong>INZ_SUBMITTED</strong> and deletes the {outcome === 'APPROVED' ? 'approval' : 'decline'} record. {outcome === 'APPROVED' && 'The visa file stays on disk (recoverable from the audit log) — only the database row is removed.'}
            </p>
            ...
            <textarea
              ...
              maxLength={500}
              disabled={submitting}
              placeholder="Why are we reverting the visa record?"
            ...
            <div className="mt-5 flex items-center justify-end gap-2">
              ...
              <button type="button" onClick={handleSubmit} disabled={!canSubmit} className="min-h-[44px] px-5 py-2 rounded-xl bg-red-700 text-white text-sm font-semibold hover:bg-red-800 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed">
                {submitting ? 'Reverting…' : 'Revert visa record'}
              </button>
            </div>
          </div>
        </div>

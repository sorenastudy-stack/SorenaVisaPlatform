    ...
    <div className="max-w-7xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3 mb-6">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-[#1E3A5F] flex items-center gap-2">
            <BarChart3 size={22} className="text-[#b28f4e]" />
            Officer Metrics
          </h1>
      ...
      <Card className="mb-6">
        <CardContent>
          <h2 className="text-lg font-bold text-[#1E3A5F] flex items-center gap-2 mb-4">
            <Activity size={18} className="text-[#b28f4e]" />
            Decisions over time
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card>
          <CardContent>
            <h2 className="text-lg font-bold text-[#1E3A5F] flex items-center gap-2 mb-4">
              <TrendingUp size={18} className="text-[#b28f4e]" />
                    ...
                    {metrics.approvalRateLeaderboard.map((o) => (
                      <tr key={o.officerId} className="hover:bg-[#FAF8F3]/50">
                        <td className="px-3 py-2">
                          <Link href={`/lia/officers/${o.officerId}`} className="font-semibold text-[#1E3A5F] hover:text-[#b28f4e]">
                            {o.fullName}
                          </Link>
                        </td>
            ...
            <details open>
              <summary className="cursor-pointer">
                <span className="text-lg font-bold text-[#1E3A5F] inline-flex items-center gap-2">
                  <AlertTriangle size={18} className="text-[#b28f4e]" />
                  Outliers
                </span>
...
function OutlierCard({ title, icon, tone, rows }: {
  ...
  const toneStyles = {
    ...
    gold: 'border-[#c9a961]/40 bg-[#c9a961]/10',
  };
  const iconTones = {
    red: 'text-red-700',
          ...
          {rows.map((r) => (
            <li key={r.id} className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                <Link href={`/lia/officers/${r.id}`} className="font-semibold text-[#1E3A5F] hover:text-[#b28f4e] truncate block text-sm">
                  {r.name}
                </Link>

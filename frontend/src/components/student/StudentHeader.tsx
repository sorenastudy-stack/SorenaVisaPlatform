        ...
        <div className="relative flex-shrink-0">
          {photoUrl ? (
            ...
            <img
              src={photoUrl}
              alt={name}
              className="w-14 h-14 rounded-full object-cover border-2 border-[#c9a961]/40 shadow-sm"
            />
          ) : (
            <div className="w-14 h-14 rounded-full flex items-center justify-center text-base font-bold bg-[#1E3A5F] text-white border-2 border-[#c9a961]/40 shadow-sm">
              {getInitials(name) || '?'}
            </div>
          )}
        </div>
